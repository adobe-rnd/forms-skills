...
- `multiLine`: set to `true` when the user intent implies a textarea, multiline, or long-text field. Omit (or `false`) for single-line text inputs.
...
- `name`: **mandatory** — must equal `id` (snake_case) and be **unique across the form**. Applies to panelcontainer, accordion, wizard, tabsontop, and fragment. `./name` is absent from `requiredKeys` in the MCP definition (Sling include not expanded), but omitting it causes AEM to persist a timestamped auto-generated name that breaks rule qualified-paths at runtime.
