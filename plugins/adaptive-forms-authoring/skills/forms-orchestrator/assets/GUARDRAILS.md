...
| Rule | Description |
|------|-------------|
| **File path** | `$FORMS_WORKSPACE/journeys/<journey>/plans/NN-<short-title>.md` |
...
| **Generation** | Plans are generated by the Planner (`references/planner/SKILL.md`) from `$FORMS_WORKSPACE/journeys/<journey>/spec.md` |
| **Decomposition guidelines** | See `references/planner/assets/GUARDRAILS.md` |

### Plan Status Tracking

Plan status is tracked in `$FORMS_WORKSPACE/.agent/handover.md` via the Plans table, managed by `forms-context-management`.
...
1. Append a row to `$FORMS_WORKSPACE/.agent/handover.md → ## Deferred Items` (create the section if absent):
...
3. When a deferred item is resolved, update its `Status` to ✅ Done in `$FORMS_WORKSPACE/.agent/handover.md`.

### Journey Completion

When all plans for a journey show ✅ Done:
- `manage-context` archives the journey to `$FORMS_WORKSPACE/.agent/history.md`
- The journey row in `$FORMS_WORKSPACE/.agent/handover.md` moves to ✅ Done
...
3. **Active plan takes priority** — if `$FORMS_WORKSPACE/.agent/handover.md` shows a 🔵 Active plan, resume it before considering new intents.
...
8. **No silent writes** — never update `$FORMS_WORKSPACE/.agent/handover.md`, deploy, or modify code without user confirmation. Exception: reading `$FORMS_WORKSPACE/.agent/handover.md` at session start is always silent (forms-context-management READ mode).
...
| Screen docs | `$FORMS_WORKSPACE/journeys/<journey>/screens/<screen>/Screen.md` |
| Custom components | `blocks/form/components/<view-type>/` (in `$FORMS_EDS_ROOT`) |
| Plans | `$FORMS_WORKSPACE/journeys/<journey>/plans/NN-<title>.md` |
| Journey spec | `$FORMS_WORKSPACE/journeys/<journey>/spec.md` |
| API refs | `$FORMS_WORKSPACE/refs/apis/<name>.<ext>` |
