...
```
"${CLAUDE_PLUGIN_ROOT}/skills/forms-orchestrator/scripts/<tool-name>" <args>
```
...
| 2 | **Always include `skills/forms-orchestrator/`** — `${CLAUDE_PLUGIN_ROOT}` points to the plugin root (the directory containing `.claude-plugin/`). Scripts live two levels deeper at `skills/forms-orchestrator/scripts/`. |
...
```
# ✅ Correct — uses CLAUDE_PLUGIN_ROOT with skills/forms-orchestrator segment
"${CLAUDE_PLUGIN_ROOT}/skills/forms-orchestrator/scripts/eds-code-sync" sync

# ❌ Wrong — missing skills/forms-orchestrator segment
"${CLAUDE_PLUGIN_ROOT}/scripts/eds-code-sync" sync

# ❌ Wrong — hardcoded absolute path
"/Users/alice/forms-skills/plugins/adaptive-forms-authoring/skills/forms-orchestrator/scripts/eds-code-sync" sync
