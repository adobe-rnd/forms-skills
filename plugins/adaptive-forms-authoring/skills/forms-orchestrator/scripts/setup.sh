...
#
# This script lives inside the orchestrator skill (skills/forms-orchestrator/scripts/)
# and creates .venv at the plugin root (alongside pyproject.toml) so the
# orchestrator package directory stays clean and distributable.
#
# Layout:
#   SCRIPTS_DIR        = skills/forms-orchestrator/scripts/   (where this file lives)
#   ORCHESTRATOR_ROOT  = skills/forms-orchestrator/           (the orchestrator skill)
#   PROJECT_ROOT       = adaptive-forms-authoring/                       (has pyproject.toml, .venv)
#
# Options:
...
ORCHESTRATOR_ROOT="$(cd "$SCRIPTS_DIR/.." && pwd)"
...
if [ "$SKIP_DEPS" = true ]; then
  ...
  if [ -d "$BRIDGE_DIR" ]; then
    if command -v npm &>/dev/null; then
      ...
      echo "      cd skills/forms-orchestrator/scripts/rule_coder/bridge && npm install"
    fi
  else
...
echo ""
echo "  Shared CLI tools (skills/forms-orchestrator/scripts/):"
