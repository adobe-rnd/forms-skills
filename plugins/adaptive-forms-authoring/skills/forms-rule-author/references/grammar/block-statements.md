...
Each param must be wrapped in an `EXPRESSION` node — bare `COMPONENT` (or any other expression type) directly in `params` is rejected by the validator with `SEMANTIC_FUNCTION_PARAM_NOT_EXPRESSION`.
