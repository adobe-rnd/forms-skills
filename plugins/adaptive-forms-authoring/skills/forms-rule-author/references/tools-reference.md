...
**Output — expression rule** (CALC_EXPRESSION, VALIDATE_EXPRESSION, etc.):
```json
{
  "fd:rules": {
    "fd:calc": ["{...raw rule JSON stringified...}"],
    "value": "compiled formula string",
...
**Output — event rule** (EVENT_SCRIPTS, e.g. fd:click):
```json
{
  "fd:rules": {
    "fd:click": ["{...raw rule JSON stringified...}"],
    "validationStatus": "valid | invalid"
  },
  "fd:events": {
    "click": ["compiled formula string"]
  }
}
```

`fd:rules` always contains the raw rule store entry from `input` plus `validationStatus` (`"valid"` or `"invalid"` based on rule validity). Expression rules additionally include the bare compiled formula key mapped from the event type (`value` for Calculate, `enabled` for Enabled, `visible` for Visibility, `validate` for Validate). Event scripts go into `fd:events` keyed by the bare event name (e.g. `click`, `valueCommit`).
