...
// src/cli/transform-content-model.js
...
function isSiteContainer(key, value) {
  return typeof value === "object" && value !== null && !Array.isArray(value) && !("fieldType" in value) && !key.startsWith("jcr:") && !key.startsWith("sling:") && !key.startsWith("fd:");
}
function jcrNodeToFormJson(jcrNode, keyName) {
  const childItems = extractChildItems(jcrNode);
  const node = {
...
function extractChildItems(jcrNode) {
  return Object.entries(jcrNode).flatMap(([k, v]) => {
    if (isFieldNode(k, v)) {
      return [jcrNodeToFormJson(v, k)];
    }
    if (isSiteContainer(k, v)) {
      return extractChildItems(v);
    }
    return [];
  });
}
function buildFormJson(formData) {
  ...
  const items = extractChildItems(formData);
  return {
    fieldType: "form",
    name: formName,
...
function isFieldNode2(node) {
  const panelTypes = /* @__PURE__ */ new Set(["panel", "form", "pageset", "pagearea"]);
...
function buildContentModelItems(itemsObj, parentQualifiedId, isAncestorRepeatable = false) {
  return sortedContentModelValues(itemsObj).flatMap(({ entry }) => {
    const props = entry.properties || {};
    if (!("fieldType" in props)) {
      return buildContentModelItems(entry.items || {}, parentQualifiedId, isAncestorRepeatable);
    }
    ...
    const isRepeatable = isPanel && props.repeatable === true;
    if (!name && isPanel) {
      if (isRepeatable) {
        return [];
      }
      return buildContentModelItems(
        entry.items || {},
        parentQualifiedId,
    ...
    return [{
      ...
      items: buildContentModelItems(
        entry.items || {},
        qualifiedId,
        isAncestorRepeatable || isRepeatable
      )
    }];
  });
}
...
(async () => {
  ...
  if (rawArgs.includes("--help") || rawArgs.includes("-h")) {
    process.stdout.write(`${[
      "Usage: transform-content-model <content-model.json> [options]",
      "       transform-content-model --content-model-file <path> [options]",
      "       transform-content-model --stdin [options]",
      "",
      "Options:",
      "  --content-model-file  Path to Sites Content API content model JSON",
      "  --stdin               Read content model from stdin",
...
`);
  ...
  for (let i = 0; i < rawArgs.length; i += 1) {
    if (rawArgs[i] === "--content-model-file") {
      i += 1;
      filePath = rawArgs[i];
    } else if (rawArgs[i] === "--output") {
      i += 1;
  ...
  if (useStdin) {
    ...
    throw new Error("Usage: transform-content-model <model.json> | --content-model-file <path> | --stdin [--output <path>]");
  }
  const treeJson = ScopeBuilder.fromContentModel(JSON.parse(raw));
