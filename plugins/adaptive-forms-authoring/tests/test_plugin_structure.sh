...
# Validates that the adaptive-forms-authoring plugin has all required files and references.
# Run from: forms-skills/plugins/adaptive-forms-authoring/
#
...
if grep -q '"name": "adaptive-forms-authoring"' ".claude-plugin/plugin.json" 2>/dev/null; then
  pass "plugin.json contains name \"adaptive-forms-authoring\""
else
  fail "plugin.json does not contain name \"adaptive-forms-authoring\""
fi
...
if grep -q '"adaptive-forms-authoring"' "$MARKETPLACE" 2>/dev/null; then
  pass "marketplace.json contains \"adaptive-forms-authoring\" entry"
else
  fail "marketplace.json does not contain \"adaptive-forms-authoring\" entry"
fi
