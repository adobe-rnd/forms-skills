...
description: Fix-branch naming, commit message structure, push, and PR-raising rules shared by fix-form-js-errors and fix-forms-java-errors.
type: reference
---
...
`<slug>` is:
- `fix-form-js-errors`: the last non-empty path segment of `FORM_URL`, slugified. Fallback: `$(basename "$REPO_PATH")`.
- `fix-forms-java-errors`: the `SHORT_CLASS` of the throw site, slugified.
...
For `fix-forms-java-errors`, the structure is:
...
| `git commit` | Perf-bot ran (fix-form-js-errors) OR Java fix applied (fix-forms-java-errors) AND `HEAD` is the fix branch |
