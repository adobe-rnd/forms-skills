...
Every sub-agent spawned by `fix-form-js-errors` or `fix-forms-java-errors` returns **exactly one** JSON object on stdout — no prose, no markdown fences. The orchestrator owns every `Edit` / `Write` / `git` / shell call; sub-agents are read-only analysers.
