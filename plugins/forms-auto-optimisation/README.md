...
| Node 20 | `auto-fix-form` + IA triage | bundled `better_sqlite3` is ABI 115; Node ≥ 21 → installed Node 20 via NVM is used automatically |
| Python 3 + `splunk-sdk` | `auto-fix-journey` Splunk / API Error modes | `pip install splunk-sdk` |
...
```json
{
  "permissions": {
    "allow": [
      ...
      "Bash(gh repo *)",
      "Bash(git clone *)",
      "Bash(bash plugins/forms-auto-optimisation/shared/scripts/*.sh *)",
