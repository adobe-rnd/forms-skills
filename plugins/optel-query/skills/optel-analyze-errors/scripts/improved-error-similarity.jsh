#!/usr/bin/env node
/**
 * Error Analysis Utility - Analyze JavaScript errors and find duplicates
 *
 * Usage:
 *   node improved-error-similarity.jsh <input-file> <output-prefix> [--threshold 0.6]
 *
 * Example:
 *   node improved-error-similarity.jsh output/errors-jan26-2026.json errors-jan26
 *
 * Output files generated:
 *   - <output-prefix>-similarity-analysis.json (detailed report)
 *   - <output-prefix>-similarity-simplified.json (simplified view)
 *   - <output-prefix>-similarity-by-file.json (grouped by file)
 */

const rawFs = require('fs');
const fs = rawFs.promises || rawFs;
...
/**
 * Extract key identifiers from error message
 */
function extractKeyIdentifiers(errorStr) {
  ...
  // Extract property names: "Cannot read property 'x' of undefined"
  ...
  // Extract quoted identifiers: 'variableName' or "variableName"
  ...
  // Extract variable names after common patterns
  const patterns = [
...
/**
 * Normalize error message for better comparison
 */
function normalizeErrorMessage(errorStr) {
  ...
  // Normalize common browser-specific phrases
...
/**
 * Tokenize error message into meaningful words
 */
...
function calculateCosineSimilarity(tokens1, tokens2) {
  ...
  tokens1.forEach(token => { freq1[token] = (freq1[token] || 0) + 1; });
  tokens2.forEach(token => { freq2[token] = (freq2[token] || 0) + 1; });
...
/**
 * Calculate error message similarity using multiple methods
 */
function calculateErrorMessageSimilarity(error1, error2) {
  if (!error1 || !error2) return 0.0;
  ...
  const scores = {
    errorTypeMatch: type1 === type2 && type1 !== 'Unknown' ? 1.0 : 0.0,
    identifierOverlap: identifiers1.length > 0 && identifiers2.length > 0
      ? calculateJaccardSimilarity(identifiers1, identifiers2)
      : 0.0,
  ...
  const weights = {
    errorTypeMatch: 0.15,
    identifierOverlap: 0.30,
    jaccardSimilarity: 0.25,
    cosineSimilarity: 0.20,
    levenshteinSimilarity: 0.10
  };
  ...
  if (scores.identifierOverlap > 0.7) {
    scores.jaccardSimilarity = Math.min(1.0, scores.jaccardSimilarity * 1.2);
    scores.cosineSimilarity = Math.min(1.0, scores.cosineSimilarity * 1.2);
  }

  const totalScore =
    (scores.errorTypeMatch * weights.errorTypeMatch) +
    (scores.identifierOverlap * weights.identifierOverlap) +
    (scores.jaccardSimilarity * weights.jaccardSimilarity) +
    (scores.cosineSimilarity * weights.cosineSimilarity) +
    (scores.levenshteinSimilarity * weights.levenshteinSimilarity);

  return Math.min(1.0, Math.max(0.0, totalScore));
}

/**
 * Parse error string to extract components
 */
function parseError(errorStr) {
  ...
  if (filename) {
    filename = filename
      .replace(/\.ACSHASH<[^>]+>/gi, '')
      .replace(/\.ACSHASH[a-f0-9]+/gi, '')
      .replace(/\.[a-f0-9]{8,16}\.(js|css|html?)/gi, '.$1')
      .replace(/\?.*$/, '')
      .split('/').pop();
  }
...
/**
 * Calculate line proximity score
 */
...
/**
 * Calculate error similarity combining location and message
 */
function calculateErrorSimilarity(error1, error2) {
  ...
  const hasFilename1 = parsed1.filename !== null && parsed1.filename !== undefined;
  const hasFilename2 = parsed2.filename !== null && parsed2.filename !== undefined;

  let filenameMatch = 0.0;
  let lineProximity = 0.0;

  if (hasFilename1 && hasFilename2) {
    filenameMatch = parsed1.filename === parsed2.filename ? 1.0 : 0.0;
  ...
  if (hasFilename1 && hasFilename2) {
    ...
    weights = { filename: 0.0, lineProximity: 0.20, errorDetails: 0.80 };
  }

  const totalScore =
    (filenameMatch * weights.filename) +
    (lineProximity * weights.lineProximity) +
    (errorDetailsSimilarity * weights.errorDetails);
...
function clusterSimilarErrors(errors, similarityThreshold = 0.6) {
  const clusters = [];
  const errorToCluster = new Map(); // Maps error index to cluster index
  const similarityMatrix = []; // Cache similarity calculations
    ...
    for (let j = i + 1; j < errors.length; j++) {
      ...
      if (similarity >= similarityThreshold) {
        ...
        if (jClusterIndex === undefined) {
          ...
          clusters[jClusterIndex] = null; // Mark as merged
        }
      }
    }
  }

  // Remove null clusters (merged ones)
...
/**
 * Find representative error for a cluster (highest count, or most similar errors)
 */
function findClusterRepresentative(cluster, errors, similarityMatrix) {
  ...
  for (const idx of cluster) {
    // Count how many errors in cluster are similar to this one
    ...
    // Score: count + similarity count (prioritize errors with more connections)
...
/**
 * Find similar errors (deduplicated by clustering)
 */
function findSimilarErrors(errors, similarityThreshold = 0.6) {
  // First, cluster similar errors
  ...
  clusters.forEach((cluster, clusterIndex) => {
    ...
    // Find representative error for this cluster
    ...
    // Collect similar errors (all other errors in the cluster)
    const similarErrors = [];
    cluster.forEach(errorIndex => {
      if (errorIndex === representativeIndex) return; // Skip self
      ...
      similarErrors.push({
        ...
        lineDiff: parsed.line !== null && otherParsed.line !== null
          ? Math.abs(parsed.line - otherParsed.line)
          : null
      });
    ...
    // Sort similar errors by similarity (highest first)
    similarErrors.sort((a, b) => b.similarity - a.similarity);

    // Add representative error as main entry
    results.push({
  ...
  // Add errors that don't belong to any cluster (no similar errors found)
...
async function main() {
  ...
  try {
    ...
    console.log(`   Groups with nearby line matches (±10 lines): ${stats.errorsWithNearbyLocationMatch}`);
    console.log('='.repeat(80));
    console.log('');

    // Determine output directory
    const outputDir = path.dirname(inputFile) || 'output';
    if (!(await pathExists(outputDir))) {
      await ensureDir(outputDir);
    }

    // Generate output files
    const baseOutputPath = path.join(outputDir, outputPrefix);
    ...
    // 2. Simplified view (all errors: with and without similar errors)
...
// This file is designed to be executed as a CLI utility
(async () => {
  await main();
})().catch((err) => {
