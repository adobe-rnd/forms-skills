#!/usr/bin/env node
/**
 * optel-query.jsh — Query AEM Operational Telemetry (RUM) data.
 *
 * Dual-runtime: runs under SLICC (auto-discovered by basename) and under
 * Node 18+ (`node optel-query.jsh <args>`).
 *
 * Env vars:
 *   DOMAINKEY_FILE  Path to JSON map {"<domain>": "<key>"}; read before each
 *                   query, written after a new admin-key fetch.
 *   RUM_ADMIN_KEY   Optional admin key used to fetch a missing domain key.
 */

const rawFs = require('fs');
const fs = rawFs.promises || rawFs;

const { DataChunks, series, facets, facetFns } = require('@adobe/rum-distiller');
const { addCalculatedProps } = require('@adobe/rum-distiller/utils.js');
...
async function readDomainkeyFile() {
  ...
  try {
    const contents = String(await fs.readFile(process.env.DOMAINKEY_FILE));
...
async function writeDomainkeyFile(domain, domainkey) {
  ...
  await fs.writeFile(process.env.DOMAINKEY_FILE, JSON.stringify(existing, null, 2));
}
...
class Loader {
  ...
  async fetchPeriod(startDate, endDate, allCheckpoints = [], interval = undefined, startDateHour = 0, endDateHour = 0) {
    ...
    if ((diff <= 1000 * 60 * 60 * 24 * 7 && !interval) || interval === 'hourly') {
    ...
    } else if ((diff <= 1000 * 60 * 60 * 24 * 31 && !interval) || interval === 'daily') {
      const days = Math.round(diff / (1000 * 60 * 60 * 24)) + 1;
      const chunkSize = 30;
    ...
    return allBundles.flatMap((b) =>
      b.rumBundles.filter((bundle) => !isBotTraffic(bundle)).map(filterEvents(allCheckpoints))
    );
  }
}

// ───── Inlined helpers (from former facets.js and series.js) ───────────
...
function formBlockLoadTime(threshold = 2 * 60 * 1000) {
...
// ───── DataChunks wiring (ported verbatim from former datachunks.js) ───

function getDataChunks(rumBundles) {
  const dataChunks = new DataChunks();
  dataChunks.load([{ rumBundles: rumBundles }]);
...
function formatINP(inpValue) {
  ...
  const ms = v < 10 ? v * 1000 : v;
  return { ms, text: `${(ms / 1000).toFixed(2)}s` };
}
...
const seriesValues = {
  lcp: (series) => {
    ...
    if (count >= CWV_MIN_COUNT && Number.isFinite(ms)) result = formatTime(ms / 1000);
    return { p75: result };
  },
  cls: (series) => {
    let result = 'N/A';
...
async function getData(domain, startDate, endDate, interval) {
  const domainKey = await fetchDomainKey(domain);
  const loader = new Loader(domain, domainKey);
  const bundles = await loader.fetchPeriod(startDate, endDate, [], interval);
  return getDataChunks(bundles);
}

async function query(domain, startDate, endDate, queryFilter = {}, series = [], interval) {
...
async function getFacetValues(domain, startDate, endDate, facetName, queryFilter = {}, series = [], interval) {
  const dataChunks = await getData(domain, startDate, endDate, interval);
  ...
  const facetValues = dataChunks.facets[facetName].map((facet) => {
    ...
    if (series) {
      series.forEach((name) => { row[name] = seriesValues[name](facet.metrics[name]); });
    }
    return row;
  });
...
const VALID_INTERVALS = ['hourly', 'daily', 'monthly'];
...
`);
  ...
  for (let i = 3; i < args.length; i++) {
    ...
    if (arg === '--query' && i + 1 < args.length) {
      try { config.query = JSON.parse(args[++i]); }
      catch (e) { console.error(`Error: Invalid JSON in --query: ${e.message}`); process.exit(1); }
  ...
  try {
    ...
    if (config.facetValues) {
      result = await getFacetValues(
        config.domain, config.startDate, config.endDate,
        config.facetValues, config.query, config.series, config.interval
      );
    } else {
      result = await query(
        config.domain, config.startDate, config.endDate,
        config.query, config.series, config.interval
      );
    }
