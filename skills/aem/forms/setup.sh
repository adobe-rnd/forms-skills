#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# AEM Forms Skills — Development Environment Setup
# Usage: ./setup.sh [--force] [--skip-deps] [--help]
#
# Sets up a Python virtual environment, installs the project in editable mode,
...
BRIDGE_DIR="$PROJECT_ROOT/skills/scripts/rule_coder/bridge"
...
echo ""
echo -e "${BOLD}AEM Forms Skills — Development Environment Setup${RESET}"
echo "─────────────────────────────────────────────────"
echo ""
...
if [ -d "$VENV_PATH" ]; then
  ok "Virtual environment already exists at ${VENV_PATH##"$PROJECT_ROOT"/}"
else
  info "Creating virtual environment …"

  if command -v uv &>/dev/null; then
    # Prefer uv — faster and can pin the Python version automatically
  ...
  ok "Virtual environment created at ${VENV_PATH##"$PROJECT_ROOT"/}"
fi
...
if [ "$SKIP_DEPS" = true ]; then
  ...
  if [ -d "$BRIDGE_DIR" ]; then
    if command -v npm &>/dev/null; then
      ...
      echo "      cd skills/scripts/rule_coder/bridge && npm install"
    fi
  else
    warn "Bridge directory not found at skills/scripts/rule_coder/bridge — skipping npm install."
  fi
fi
...
echo ""
echo "  Shared CLI tools (skills/bin/):"
...
echo ""
echo "  Skill-local CLI tools (skills/<domain>/<skill>/scripts/):"
echo -e "    ${CYAN}form-validate${RESET}   — Validate form JSON        (create-form)"
echo -e "    ${CYAN}cct-create${RESET}      — Scaffold custom component (create-component)"
echo -e "    ${CYAN}api-skill${RESET}       — Generate API from cURL    (manage-apis)"
echo -e "    ${CYAN}form-sync${RESET}       — Sync forms with AEM       (sync-forms)"
echo -e "    ${CYAN}eds-code-sync${RESET}   — Sync EDS code via GitHub  (sync-eds-code)"
echo -e "    ${CYAN}git-sandbox${RESET}     — Isolated Git workspace    (git-sandbox)"
echo -e "    ${CYAN}scaffold-form${RESET}   — Scaffold empty form JSON  (scaffold-form)"
echo ""
