...
description: >
  ...
  Used by the orchestrator to resolve pipeline phase targets and direct domain routing.
...
> 1. **Pipeline phase execution** — a pipeline phase declares `domain: analysis, skill: analyze-requirements` → this registry resolves the path
> 2. **Direct domain routing** — no pipeline matched, user intent maps to a single domain → this registry routes to it
...
How pipeline phases resolve to domain skills (exact-match rules, hard-stop on missing domain/skill): **[`assets/phase-resolution.md`](assets/phase-resolution.md)**
...
| Phase resolution algorithm | `assets/phase-resolution.md` |
