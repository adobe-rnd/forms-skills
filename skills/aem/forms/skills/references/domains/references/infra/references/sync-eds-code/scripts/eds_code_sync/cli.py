    ...
    try:
        ...
        try:
            ...
            if e.status == 404:
                ...
                click.echo(f"   Either the repository doesn't exist or you don't have access.")
                click.echo()
                click.echo("Possible issues:")
                click.echo(f"  • Repository name might be wrong: {config.github_repo}")
                click.echo(f"  • Repository might be private and your token lacks access")
                click.echo(f"  • Token might not have 'repo' scope for private repositories")
                sys.exit(1)
            elif e.status == 403:
                click.secho(f"❌ Access forbidden!", fg="red", bold=True)
                click.echo(f"   Your token doesn't have permission to access this repository.")
                click.echo()
                click.echo("To fix this:")
                click.echo("  1. Go to: https://github.com/settings/tokens")
                click.echo("  2. Make sure your token has 'repo' scope (for private repos)")
        ...
        click.echo()
        click.secho("🎉 Everything looks good! You're ready to sync.", fg="green", bold=True)
...
def delete_branch(branch: str, force: bool, verbose: bool):
    ...
    try:
        ...
        if branch == repo.default_branch or branch in ['main', 'master']:
            click.secho(f"❌ Cannot delete default/main branch: {branch}", fg="red", bold=True)
            click.echo("This is a safety check to prevent accidental deletion of main branches.")
            sys.exit(1)
        ...
        try:
            ...
            if verbose:
                click.echo(f"Branch URL was: https://github.com/{config.github_repo}/tree/{branch}")
            
        except GithubException as e:
            if e.status == 422:
                click.secho(f"❌ Cannot delete branch: {e.data.get('message', 'Unknown error')}", fg="red")
