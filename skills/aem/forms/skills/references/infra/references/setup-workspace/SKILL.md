...
description: >
  ...
  verifies system requirements, and performs first-run setup. Creates .env interactively by
  asking the user for each credential one at a time.
...
3. **Collect credentials ONE AT A TIME** — do not dump a template and ask the user to fill it in; ask each value individually in a conversational flow
4. **Never hardcode credentials** — always write to `.env`; never commit `.env` to version control
5. **Verify system requirements** — confirm Node.js 18+ and Python 3.10+ are available
6. **Test credentials after collecting them** — use `eds-code-sync test` to verify AEM and GitHub connectivity
7. **Accept "skip"** — if the user doesn't have a value yet, write a placeholder comment and move on
...
For example, if the user's cwd is `/Users/alice/projects` and they chose the name `loan-app`, the value is:
...
Confirm to the user:
> "Created workspace at `/Users/alice/projects/loan-app`. Now let's configure your credentials."
...
| 2 | `AEM_TOKEN` | "Paste your AEM bearer token." | From AEM Author → Developer Console (`https://dev-console-pXXXX-eYYYY.adobeaemcloud.com/dc/`) → Integrations → Local Token → Get Local Development Token. Tokens expire after 24h. |
| 3 | `GITHUB_URL` | "What is the full GitHub URL for your EDS repo?" | e.g. `https://github.com/owner/repo` |
| 4 | `GITHUB_REPO` | "What is the repo in `owner/repo` format?" | e.g. `adobe/my-eds-repo`. You can usually derive this from the URL they just gave. If obvious, auto-fill and confirm. |
| 5 | `GITHUB_TOKEN` | "Paste your GitHub personal access token." | Classic PAT with `repo` scope. Starts with `ghp_`. Generate at github.com/settings/tokens. |
| 6 | `AEM_WRITE_PATHS` | "Which AEM content paths should be writable? (comma-separated)" | e.g. `/content/forms/af/my-team` |
...
If the user says skip for any variable, write it as a commented placeholder:

```
# AEM_TOKEN=<paste-your-bearer-token-here>
```

And note to the user which values are still pending.
...
Append all collected credentials to `<workspace>/.env` (below the `FORMS_WORKSPACE` line written in Step 2):
...
AEM_TOKEN=<collected-value>

# ── GitHub EDS Repo ──────────────────────────────────────
GITHUB_URL=<collected-value>
GITHUB_REPO=<collected-value>
GITHUB_TOKEN=<collected-value>
...
**Never echo secrets back** to the user after writing the file. Just confirm:
...
| `api-manager` | `bin/` (shared) | Manage API definitions, generate typed JS clients |
...
| `rule-transform` | `bin/` (shared) | Transform form JSON for rule editing |
| `rule-validate` | `bin/` (shared) | Validate rule JSON against grammar |
| `rule-save` | `bin/` (shared) | Save rules back to rule store |
| `rule-grammar` | `bin/` (shared) | Print rule grammar reference |
| `parse-functions` | `bin/` (shared) | Parse custom function JSDoc annotations |
