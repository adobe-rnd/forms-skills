...
function getPythonPath() {
  ...
  // Check for plugin's own bin/python3 (handles venv bootstrap)
  const pluginRoot = path.resolve(__dirname, "../../../../");
  const pluginPython = path.join(pluginRoot, "bin", "python3");
  if (fs.existsSync(pluginPython)) {
    candidates.push(pluginPython);
  }

  // Check for .venv in plugin root
  candidates.push(path.join(pluginRoot, ".venv", "bin", "python"));
